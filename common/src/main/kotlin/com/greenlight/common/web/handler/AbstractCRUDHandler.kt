...
abstract class AbstractCRUDHandler<T : Any, D, I>(private val readWriteService: AbstractCoroutineReadWriteService<T, D, I>) {
    ...
    open suspend fun findAll(request: ServerRequest): ServerResponse {
        val items = readWriteService.findAll()
        val bodyFromPublisher = BodyInserters.fromPublisher(items.asFlux(),
            object : ParameterizedTypeReference<T>() {})
        return ServerResponse.ok()
            .contentType(MediaType.APPLICATION_JSON).body(bodyFromPublisher).awaitFirst()
    }

    open suspend fun findOne(request: ServerRequest): ServerResponse {
        val id = extractId(request)
    ...
    open suspend fun delete(request: ServerRequest): ServerResponse {
        val id = extractId(request)
